...
#tukey_min<-
  tukey_hsd(anova_model_heritage)
min_F <- aov_min$statistic
min_p <- aov_min$p.value

```

To test this statistically, we conducted a one-way analysis of variance (ANOVA test) to explore weather some family language strategies contributed more variance to the amount of exposure babies got in French, English and/or a heritage language. 

In terms of French exposure, we found a statistically significant difference in how much variance the family language strategies contribute (F=`r fre_F`, p= `r fre_P`). We then conducted a Hukey HSD post-hoc analysis to determine where specifically were the differences in variance between the family language strategies (See Table 2). The post-hoc analyses showed that the one parent bilingual strategy statistcally singificantly accounted for more variance in the French exposure when compared to the both parents bilingual, the one-parent-one-language and the one langauge at home strategies.

In terms of English exposure, we found a statistically significant difference in how much variance the family language strategies contribute (F=`r eng_F`, p= `r eng_P`). We then conducted a Hukey HSD post-hoc analysis to determine where specifically were the differences in variance between the family language strategies (See Table 3). Contrary to what we found for the French exposure, the post-hoc analyses showed thatthe one-parent-one-language and one language at home strategies accounted for more variance in English exposure than the one parent bilingual strategy. This makes sense as most of the families in our sample were French-English bilinguals, and thus English exposure was in most cases a complement to French exposure.

Finally, in terms of exposure to a heritage language, we found an approaching statistically significant difference in how much variance the family language strategies contribute (F=`r min_F`, p= `r min_P`). The fact that the the difference between strategies only approaches significance might be due to a lack of power, since the sample of families who used a heritage language was relatively small. We thus decided to also conduct a Hukey HSD post-hoc analysis. The post-hoc analysis showed that the one language at home strategy statistically significantlly contributes more variance to the exposure to a heritage language.
