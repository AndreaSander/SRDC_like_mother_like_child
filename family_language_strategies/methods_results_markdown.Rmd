...
for_balance<- for_beta_blc%>% 
  select(age, exp_l2, strategy, anon_baby_id, caregiver_1_french_use, caregiver_1_english_use, french_abs_dif, english_abs_dif) %>%
  filter(exp_l2 != "NA") %>% 
...
mom_beta_blc_fre<- glmmTMB(exp_l2 ~ french_abs_dif, data=for_balance, family= beta_family(link="logit"), dispformula = ~ french_abs_dif)
mom_beta_blc_eng<- glmmTMB(exp_l2 ~ english_abs_dif, data=for_balance, family= beta_family(link="logit"), dispformula = ~ english_abs_dif)
...
dad_beta_blc_fre<- glmmTMB(exp_l2 ~ french_abs_dif, data=for_balance, family= beta_family(link="logit"), dispformula = ~ french_abs_dif)
dad_beta_blc_eng<- glmmTMB(exp_l2 ~ english_abs_dif, data=for_balance, family= beta_family(link="logit"), dispformula = ~ english_abs_dif)

#Summary of FATHER models
summary(dad_beta_eng)
summary(dad_beta_fre)
summary(dad_beta_blc_fre)
sumamry(dad_beta_blc_eng)
...
parents_aditive_blc <- glmmTMB(exp_l2 ~ french_abs_dif + english_abs_dif, data=for_balance, family= beta_family(link="logit"), dispformula = ~ french_abs_dif  + english_abs_dif)
...
for_careg_graphs$caregiver_1_french_use <- factor(for_careg_graphs$caregiver_1_french_use, levels= c("never","sometimes", "regularly"))
for_careg_graphs$caregiver_1_english_use <- factor(for_careg_graphs$caregiver_1_english_use, levels= c("never","sometimes", "regularly"))
for_careg_graphs$caregiver_2_french_use <- factor(for_careg_graphs$caregiver_2_french_use, levels= c("never","sometimes", "regularly"))
for_careg_graphs$caregiver_2_english_use <- factor(for_careg_graphs$caregiver_2_english_use, levels= c("never","sometimes", "regularly"))

figure_10_a <-ggplot(for_careg_graphs, aes( x=caregiver_1_french_use , y=fre_exp, fill=caregiver_1_french_use)) +
  geom_boxplot() +
  theme_bw() +
       theme(axis.text.x = element_blank(), axis.text.y = element_text(size=20),axis.title = element_text(size=20), plot.title = element_text(size=20), legend.position = "none") +
  labs(title= "A. Mothers' French use on French exposure")

figure_10_b <-ggplot(for_careg_graphs, aes( x=caregiver_1_english_use , y=eng_exp, fill=caregiver_1_english_use)) +
  geom_boxplot() +
  theme_bw() +
       theme(axis.text.x = element_blank(), 
         axis.text.y = element_text(size=20),axis.title = element_text(size=20), plot.title = element_text(size=20), legend.text= element_text(size=20),legend.title= element_text(size=20))+
  labs(title= "B. Mothers' English use on English exposure") +
  scale_fill_discrete(name = "Parental language use categories")

figure_10_c <-ggplot(for_careg_graphs, aes( x=caregiver_2_french_use , y=fre_exp, fill=caregiver_2_french_use)) +
  geom_boxplot() +
  theme_bw() +
       theme(axis.text.x = element_blank(), axis.text.y = element_text(size=20),axis.title = element_text(size=20), plot.title = element_text(size=20), legend.position = "none")+
  labs(title= "C. Fathers' French use on French exposure")

figure_10_d <-ggplot(for_careg_graphs, aes( x=caregiver_2_english_use , y=eng_exp, fill=caregiver_2_english_use)) +
  geom_boxplot() +
  theme_bw() +
       theme(axis.text.x = element_blank(), axis.text.y = element_text(size=20),axis.title = element_text(size=20), plot.title = element_text(size=20), legend.position = "none")+
  labs(title= "D. Fathers' English use on English exposure")

figure_10<- figure_10_a + figure_10_b + figure_10_c + figure_10_d 
...
## Seems like linear models have the same results as beta models 

#STRATEGY LINEAR MODELS
lm_strat_eng<- lm(eng_exp ~ strategy, data=for_beta_eng)
lm_strat_fre <- lm(fre_exp ~ strategy, data=for_beta_fre)
lm_strat_blc<- lm(exp_l2 ~ strategy, data=for_balance)
...
summary(lm_strat_blc) #Same pattern of results 

#AGE MODELS

#just age models
lm_just_age_eng<- lm(eng_exp ~ age, data=for_beta_eng)
lm_just_age_fre<- lm(fre_exp ~ age, data=for_beta_fre)
lm_just_age_blc<- lm(exp_l2 ~ age, data=for_balance)
...
summary(lm_just_age_blc) #Same pattern of results for beta anf lm

#age plus strategy models
lm_age_plus_eng<- lm(eng_exp ~ age + strategy, data=for_beta_eng) # Different results, in lm one parent bilingual sig
lm_age_plus_fre<- lm(fre_exp ~ age + strategy, data=for_beta_fre) 
lm_age_plus_blc<- lm(exp_l2 ~ age + strategy, data=for_balance)  

#Summary of just age AGE models
summary(lm_age_plus_eng) # Different results, in lm one parent bilingual sig
summary(lm_age_plus_fre) # Same as avobe
summary(lm_age_plus_blc) # Different results, in beta models age is approaching sig, and opol is sig, in lm one parent bi, and opol are sig

#age by strategy models
lm_age_by_eng<- lm(eng_exp ~ age * strategy, data=for_beta_eng)
lm_age_by_fre<- lm(fre_exp ~ age * strategy, data=for_beta_fre)
lm_age_by_blc<- lm(exp_l2 ~ age * strategy, data=for_balance)

#Summary of just age AGE models
summary(lm_age_by_eng) #same pattern of results
summary(lm_age_by_fre) #SImilar pattern of results
summary(lm_age_by_blc) #Difference, one parent bilingual strategy sig for lm 


#MOTHER MODELS
lm_mom_beta_eng<- lm(eng_exp ~ caregiver_1_english_use, data=for_beta_eng)
lm_mom_beta_fre<- lm(fre_exp ~ caregiver_1_french_use, data=for_beta_fre)
lm_mom_beta_blc_fre<- lm(exp_l2 ~ french_abs_dif, data=for_balance)
lm_mom_beta_blc_eng<- lm(exp_l2 ~ english_abs_dif, data=for_balance)

#Summary of MOTHER models
summary(lm_mom_beta_eng) #Similar pattern of results
summary(lm_mom_beta_fre) #same pattern of results
summary(lm_mom_beta_blc_fre) #same pattern of results
summary(lm_mom_beta_blc_eng) #same pattern of results


#FATHER MODELS 
lm_dad_beta_eng<- lm(eng_exp ~ caregiver_2_english_use, data=for_beta_eng) 
lm_dad_beta_fre<- lm(fre_exp ~ caregiver_2_french_use, data=for_beta_fre)
lm_dad_beta_blc_fre<- lm(exp_l2 ~ french_abs_dif, data=for_balance)
lm_dad_beta_blc_eng<- lm(exp_l2 ~ english_abs_dif, data=for_balance)

#Summary of FATHER models
summary(lm_dad_beta_eng) #Same pattern of results
summary(lm_dad_beta_fre) #same pattern of results
summary(lm_dad_beta_blc_fre) #Same pattern of results
summary(lm_dad_beta_blc_eng) #same pattern of results


#MOTHER AND FATHER MODELS
lm_parents_aditive_eng<- lm(eng_exp ~ caregiver_2_english_use + caregiver_1_english_use, data=for_beta_eng)

lm_parents_aditive_fre<- lm(fre_exp ~ caregiver_2_french_use + caregiver_1_french_use, data=for_beta_fre)
lm_parents_aditive_blc <- lm(exp_l2 ~ french_abs_dif + english_abs_dif, data=for_balance)

#Summary of Mother and Father models
summary(lm_parents_aditive_eng) #same pattern of results
summary(lm_parents_aditive_fre) #same pattern of results
summary(lm_parents_aditive_blc) #similar pattern but the french abs difference is significant for the beta model and not for the lm


 ## LINEAR MODEL COMPARISON
#These R squared measures do make sense
performance::compare_performance(lm_mom_beta_eng, lm_parents_aditive_eng)
```
##Categorical parental input predictors

```{r}

#For careg graphs already has the categorical levels, and the levels have been re-ordered in the chunk above

# Dataframe for english exposure
for_cat_eng <- for_careg_graphs %>% 
  select(age, eng_exp, strategy, anon_baby_id, caregiver_1_french_use, caregiver_1_english_use, caregiver_2_french_use,caregiver_2_english_use) %>% 
  filter(eng_exp != "NA") %>% 
  mutate(eng_exp = eng_exp/100) %>%
  drop_na()
...
#Dataframe for balance 
for_cat_balance<- for_careg_graphs %>% 
  select(age, exp_l2, strategy, anon_baby_id, caregiver_1_french_use, caregiver_1_english_use, french_abs_dif, english_abs_dif) %>% ##have to make the difference measure here
  filter(exp_l2 != "NA") %>% 
  mutate(exp_l2 = exp_l2/100,
         strategy = as.factor(strategy)) %>%
  drop_na()
...
#MOTHER MODELS
cat_mom_eng<- lm(eng_exp ~ caregiver_1_english_use, data=for_cat_eng)
cat_mom_fre<- lm(fre_exp ~ caregiver_1_french_use, data=for_cat_fre)
cat_mom_beta_blc_fre<- lm(exp_l2 ~ french_abs_dif, data=for_balance)
cat_mom_beta_blc_eng<- lm(exp_l2 ~ english_abs_dif, data=for_balance)

#Summary of MOTHER models
summary(cat_mom_eng) #Similar pattern of results
summary(cat_mom_fre) #same pattern of results
summary(lm_mom_beta_blc_fre) #same pattern of results
summary(lm_mom_beta_blc_eng) #same pattern of results
...
summary(lm_parents_aditive_eng) #same pattern of results
summary(lm_parents_aditive_fre) #same pattern of results
summary(lm_parents_aditive_blc)
...
#Reordering levels so legend in the graph is in order.
prop_strategy_FH$strategy<- factor(prop_strategy_FH$strategy, levels= c("one-parent-bilingual","both-parents-bilingual", "one-parent-one-lang", "one-language-at-home", "single-parent"))

prop_strategy_FH<- prop_strategy_FH %>%
 arrange(desc(prop))

#Waffle plot
V1_FH<- setNames(prop_strategy_FH$n, c( "one-parent-bilingual","both-parents-bilingual","one-parent-one-lanaguage", "one-language-at-home", "single-parent"))

figure_6_a<- waffle(parts = V1_FH, rows=5, colors = c(my_yellow, my_orange, my_blue, my_green, my_purple), keep = T, title = "% Family Strategies for the French-heritage sample", size=0.6)
...
#Reordering levels so legend in the graph is in order.
prop_strategy_EH$strategy<- factor(prop_strategy_EH$strategy, levels= c("one-parent-bilingual","both-parents-bilingual","one-parent-one-lang", "one-language-at-home", "single-parent"))

prop_strategy_EH<- prop_strategy_EH %>%
 arrange(desc(prop))

#Waffle plot
V1_EH<- setNames(prop_strategy_EH$n, c("one-parent-bilingual", "both-parents-bilingual", "one-parent-one-lanaguage", "single-parent"))

figure_6_b<- waffle(parts = V1_EH, rows=5, colors = c(my_yellow, my_orange, my_blue, my_purple), keep = T, title = "% Family Strategies for the English-heritage sample", size=0.6)
...
```{r study 2 chisq tests}
for_chisq_FH <- prop_strategy_FH %>%
  filter(!strategy %in% c("single-parent", "one-language-at-home")) 

for_chisq_EH <- prop_strategy_EH %>%
  filter(!strategy== "single-parent")

xsq_test_FH<- chisq.test(for_chisq_FH$n)
xsq_test_EH<- chisq.test(for_chisq_EH$n)
...
FH_for_anova_graphs<-FEH %>%
  filter(bilingual_type == "fre-her")

FH_for_anova_graphs$strategy<- factor(FH_for_anova_graphs$strategy, levels= c("both-parents-bilingual", "one-parent-bilingual", "one-parent-one-lang", "one-language-at-home", "single-parent"))

FH_for_anova_graphs <- FH_for_anova_graphs %>%
  filter(!strategy %in% c("single-parent", "one-language-at-home"))
  

figure_7_a <-ggplot(FH_for_anova_graphs, aes( x=strategy , y=fre_exp, fill = strategy )) +
  stat_summary(geom="bar")+
  stat_summary() +
  ylim(0,100) +
   scale_fill_manual (name = "Family Strategy", values =c(my_orange, my_yellow, my_blue)) +
...
figure_7_b <-ggplot(FH_for_anova_graphs, aes( x=strategy , y=min_exp, fill = strategy )) +
  stat_summary(geom="bar")+
  stat_summary() +
  ylim(0,100) +
   scale_fill_manual (name = "Family Strategy", values =c(my_orange, my_yellow, my_blue)) +
  theme_bw() +
       theme(axis.text.x = element_blank(), axis.text.y = element_text(size=20),axis.title = element_text(size=20), plot.title = element_text(size=20), legend.position = "none")+
  labs(title= "B. Mean heritage exposure by strategy")

figure_7_c <-ggplot(FH_for_anova_graphs, aes( x=strategy , y=exp_l2, fill = strategy )) +
  stat_summary(geom="bar")+
  stat_summary() +
  ylim(0,50) +
   scale_fill_manual (name = "Family Strategy", values =c(my_orange, my_yellow, my_blue)) +
...
figure_7<- figure_7_a + figure_7_b + figure_7_c 

#### English Heritage

EH_for_anova_graphs<-FEH %>%
  filter(bilingual_type == "eng-her")

EH_for_anova_graphs$strategy<- factor(EH_for_anova_graphs$strategy, levels= c("both-parents-bilingual", "one-parent-bilingual", "one-parent-one-lang", "single-parent"))

EH_for_anova_graphs <- EH_for_anova_graphs %>%
  filter(!strategy %in% c("single-parent", "one-language-at-home"))
  

figure_8_a <-ggplot(EH_for_anova_graphs, aes( x=strategy , y=eng_exp, fill = strategy )) +
  stat_summary(geom="bar")+
  stat_summary() +
  ylim(0,100) +
   scale_fill_manual (name = "Family Strategy", values =c(my_orange, my_yellow, my_blue)) +
...
figure_8_b <-ggplot(EH_for_anova_graphs, aes( x=strategy , y=min_exp, fill = strategy )) +
  stat_summary(geom="bar")+
  stat_summary() +
  ylim(0,100) +
   scale_fill_manual (name = "Family Strategy", values =c(my_orange, my_yellow, my_blue)) +
...
figure_8_c <-ggplot(EH_for_anova_graphs, aes( x=strategy , y=exp_l2, fill = strategy )) +
  stat_summary(geom="bar")+
  stat_summary() +
  ylim(0,50) +
   scale_fill_manual (name = "Family Strategy", values =c(my_orange, my_yellow, my_blue)) +
...
figure_8<- figure_8_a + figure_8_b + figure_8_c 
...
FH_anova_model_french <- aov(fre_exp ~ strategy, data= FH_for_anovas)
FH_anova_model_heritage <- aov(min_exp ~ strategy, data= FH_for_anovas)
FH_anova_model_balance <- aov(exp_l2 ~ strategy, data= FH_for_anovas)

FH_aov_fre<- tidy(FH_anova_model_french)
FH_aov_her<- tidy(FH_anova_model_heritage)
FH_aov_blc<- tidy(FH_anova_model_balance)


##### English-Heritage sample
...
EH_anova_model_english <- aov(eng_exp ~ strategy, data= EH_for_anovas)
EH_anova_model_heritage <- aov(min_exp ~ strategy, data= EH_for_anovas)
EH_anova_model_balance <- aov(exp_l2 ~ strategy, data= EH_for_anovas)

EH_aov_eng<- tidy(EH_anova_model_english)
EH_aov_her<- tidy(EH_anova_model_heritage)
EH_aov_blc<- tidy(EH_anova_model_balance)

EH_tukey_eng<- TukeyHSD (EH_anova_model_english)
EH_tukey_her<- TukeyHSD (EH_anova_model_heritage)
```
