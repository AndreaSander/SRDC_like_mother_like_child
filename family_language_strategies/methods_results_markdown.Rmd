...
visit_length<- longitudinal_data %>%
  ...
  group_by(anon_baby_id) %>%
  fill(first_visit_date, second_visit_date, .direction="updown") %>%
  distinct() %>%
  ungroup() %>%
...
tukey_eng<-tukey_hsd(anova_model_english)
...
tukey_min<-tukey_hsd(anova_model_heritage)
