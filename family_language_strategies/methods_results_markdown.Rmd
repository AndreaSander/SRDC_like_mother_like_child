...
ggplot(data = E_l1, aes(x=care1_l1, fill= care1_l1)) +
  geom_bar() +
  facet_grid(cols = vars(strategy))
...
figure_3_d <-ggplot(FE_for_anova_graphs, aes( x=age , y=exp_l2, color=strategy )) +
  geom_smooth(alpha=0.2)+
  scale_color_manual (name = "Family Strategy", values =c(my_orange, my_yellow, my_blue)) +
  theme_bw() +
  ylim(0, 50)+
...
figure_3<- figure_3_a + figure_3_b + figure_3_d
...
strat_fre <- glmmTMB(eng_exp ~ strategy, data=for_beta_fre, family= beta_family(link="logit"), dispformula = ~ strategy)
