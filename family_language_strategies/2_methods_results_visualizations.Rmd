...
figure_1<- waffle(parts = V1_FE, rows=13, colors = c(my_orange, my_yellow, my_blue, my_green, my_purple), keep = T, size=0.6)+
    annotate("richtext", x=c(4,11,15), y=8, label=c("52%", "24%", "19%"), size=10) +
  annotate("richtext", x=17, y=6, label= "3%", size=10)+
  annotate("richtext", x=17, y=10, label= "2%", size=10)+
...
figure_2 <- FE_p1+ 
      scale_fill_manual(name= "Family Language Strategies", values = c(my_orange, my_yellow, my_blue))

figure_2<- figure_2 +
 # geom_sankey_label(size=10, color="black", fill="white")+
  theme_bw()+
    annotate("richtext", x=c(1,1,1), y=c(15,5,-8), label=FE_node_counts$count, size=10)+ #Adding the N at time 1
  annotate("richtext", x=c(2,2,2), y=c(15,6,-8), label=FE_next_node_counts$count, size=10)+ #Adding the N at time 2
...
figure_3_a <- ggplot(mom_long_fre_s , aes(x = x, 
  ...
  theme_bw()+
  theme(axis.text.x = element_text(size=30),legend.position = "none", plot.title = element_text(size=35), axis.title=element_blank(), plot.subtitle = element_text(size = 20)) +
  labs(title= "A. Mothers' change of language use", subtitle = "French")+
      annotate("richtext", x=c(1,1,1), y=c(5,-9,-20), label=mom_fre_node_counts$count, size=10)+ #Adding the N at time 1
  annotate("richtext", x=c(2,2,2), y=c(5,-9,-20), label=mom_fre_next_node_counts$count, size=10) #Adding the N at time 2
...
figure_3_b <- ggplot(mom_long_eng_s , aes(x = x, 
  ...
  theme_bw()+
  theme(axis.text.x = element_text(size=30), plot.title = element_text(size=35), axis.title = element_blank(), legend.text = element_text(size=30), legend.title = element_text(size=30), plot.subtitle = element_text(size = 20))+
  labs(title= "B. Mothers' change of language use", subtitle = "English")+
      annotate("richtext", x=c(1,1,1), y=c(6,-10,-20), label=mom_eng_node_counts$count, size=10)+ #Adding the N at time 1
  annotate("richtext", x=c(2,2,2), y=c(5,-10,-20), label=mom_eng_next_node_counts$count, size=10) #Adding the N at time 2
...
figure_4_a <- ggplot(dad_long_fre_s , aes(x = x, 
  ...
  geom_sankey(flow.color = "black") +
    scale_fill_manual(values = c("#dac291","#41ae76", "#75638b"))+
  theme_bw()+
  theme(axis.text.x = element_text(size=30), legend.position = "none", plot.title = element_text(size=35), plot.subtitle = element_text(size = 20))+
  labs(title= "A. Fathers' change of language use", subtitle = "French")+
      annotate("richtext", x=c(1,1,1), y=c(5,-10,-20), label=dad_fre_node_counts$count, size=10)+ #Adding the N at time 1
  annotate("richtext", x=c(2,2,2), y=c(5,-10,-20), label=dad_fre_next_node_counts$count, size=10) #Adding the N at time 2
...
figure_4_b <- ggplot(dad_long_eng_s , aes(x = x, 
  ...
  labs(title= "B. Fathers' change of language use", subtitle = "English")+
      annotate("richtext", x=c(1,1,1), y=c(4,-10,-22), label=dad_eng_node_counts$count, size=10)+ #Adding the N at time 1
  annotate("richtext", x=c(2,2,2), y=c(4,-10,-22), label=dad_eng_next_node_counts$count, size=10) #Adding the N at time 2
...
## Study 1 supplementary analyses: linear models
```{r}

#Creating relevant variables for supplementary analyses
...
#How bilingual mothers versus father were, and how proficient in their bilingualism, and by strategy.



#histogram of proficiency distribution 

##  French

hist(for_sup$mother_french_aoa)
hist(for_sup$father_french_aoa)


## English
hist(for_sup$mother_english_aoa)
hist(for_sup$father_english_aoa)


## By strategy accent
##### proficiency (accent) by strategy
   
    French_mother_proficiency <- for_sup %>%
   filter(!strategy == "single-parent")%>%
   drop_na(strategy)%>%
  group_by(strategy, mother_french_accent) %>%
   drop_na(mother_french_accent)%>%
  tally()%>%
    mutate(prop = n/sum(n)) %>%
  mutate(perc= prop*100)
    
        French_father_proficiency <- for_sup %>%
   filter(!strategy == "single-parent")%>%
   drop_na(strategy)%>%
  group_by(strategy, father_french_accent) %>%
   drop_na(father_french_accent)%>%
  tally()%>%
    mutate(prop = n/sum(n)) %>%
  mutate(perc= prop*100)
 
    English_mother_proficiency <- for_sup %>%
   filter(!strategy == "single-parent")%>%
   drop_na(strategy)%>%
  group_by(strategy, mother_english_accent) %>%
   drop_na(mother_english_accent)%>%
  tally()%>%
    mutate(prop = n/sum(n)) %>%
  mutate(perc= prop*100)
    
        English_father_proficiency <- for_sup %>%
   filter(!strategy == "single-parent")%>%
   drop_na(strategy)%>%
  group_by(strategy, father_english_accent) %>%
   drop_na(father_english_accent)%>%
  tally()%>%
    mutate(prop = n/sum(n)) %>%
  mutate(perc= prop*100)
         
        
ggplot(French_mother_proficiency, aes(y=perc, x=strategy, fill=mother_french_accent)) +
   geom_col(position = "fill") +
   scale_fill_brewer(palette="Dark2")+
   labs(title= "French-English sample maternal french proficiency by strategy")+
   theme(axis.text.x = element_text(angle = 25))
 
ggplot(French_father_proficiency, aes(y=perc, x=strategy, fill=father_french_accent)) +
   geom_col(position = "fill") +
   scale_fill_brewer(palette="Dark2")+
   labs(title= "French-English sample paternall french proficiency by strategy")+
   theme(axis.text.x = element_text(angle = 25))

ggplot(English_mother_proficiency, aes(y=perc, x=strategy, fill=mother_english_accent)) +
   geom_col(position = "fill") +
   scale_fill_brewer(palette="Dark2")+
   labs(title= "French-English sample maternal english proficiency by strategy")+
   theme(axis.text.x = element_text(angle = 25))
 
ggplot(English_father_proficiency, aes(y=perc, x=strategy, fill=father_english_accent)) +
   geom_col(position = "fill") +
   scale_fill_brewer(palette="Dark2")+
   labs(title= "French-English sample paternal english proficiency by strategy")+
   theme(axis.text.x = element_text(angle = 25))

### proficiency by strategy scatter plots
ggplot(for_sup, aes(x= strategy, y = mother_french_aoa))+
  geom_jitter()
ggplot(for_sup, aes(x= strategy, y = father_french_aoa))+
  geom_jitter()
ggplot(for_sup, aes(x= strategy, y = mother_english_aoa))+
  geom_jitter()
ggplot(for_sup, aes(x= strategy, y = father_english_aoa))+
  geom_jitter()


############ Statistical models for proficiency
...
for_sup_lms<- for_sup %>% 
  ...
  filter(!strategy == "OneLanguageAtHome") %>%
  mutate(fre_exp = fre_exp/100, eng_exp = eng_exp/100, exp_l2 = exp_l2/100)  
#Here we only have mother-family families left so no need to filter

#Only proficiency models
lm_aoa_fre<- lm(eng_exp ~  mother_english_aoa + father_english_aoa, data=for_sup_lms)
lm_aoa_eng<- lm(fre_exp ~ mother_french_aoa + father_french_aoa, data=for_sup_lms)
#Summary of proficiency models
summary(lm_aoa_eng) #only mother
summary(lm_aoa_fre) #both, but mother 3 times more
...
summary(lm_stratBYaoa_fre) # mother french aoa also has a significant effect
summary(lm_stratBYacc_eng) #again mother accent is significant = when mother has a perceptible accent exp decreases 
summary(lm_stratBYacc_fre) #Same, mother accent is significant. Not fathers
...
lm_parentsBYacc_fre<- lm(fre_exp ~ caregiver_2_french_use + caregiver_1_french_use + mother_french_accent + father_french_accent, data=for_sup_lms)

#summary of CAREGIVER Linear models
summary(lm_parentsBYaoa_eng) #mother aoa still significant but not fathers'
summary(lm_parentsBYaoa_fre) #no effect of proficiency
summary(lm_parentsBYacc_eng) #mother acent is significant, but not fathers
summary(lm_parentsBYacc_fre) #mother accent somewhat significant. Not fathers

##Models comparison

performance::compare_performance(lm_parents_aditive_fre, lm_strat_fre, lm_parentsBYaoa_fre, lm_stratBYaoa_fre)
#Additive is still best fit, with proficiency it accounts for 2% more variance. 
performance::compare_performance(lm_parents_aditive_eng, lm_strat_eng, lm_parentsBYacc_eng, lm_stratBYaoa_eng)
#in english performance data does make it the best fitting model, also accounts for 2% more variance, but much much more variance accounted for strat w/proficiency than without


## Model where language use is predicted by proficiency. assign codes to language use 0 1 2.

for_lang_use<- for_sup_lms %>%
...
## AOA Scatter plots
ggplot(for_lang_use, aes(x= caregiver_1_french_use, y = mother_french_aoa))+
  geom_jitter()

ggplot(for_lang_use, aes(x= caregiver_1_french_use, y = father_french_aoa))+
  geom_jitter()

ggplot(for_lang_use, aes(x= caregiver_2_french_use, y = mother_french_aoa))+
  geom_jitter()

ggplot(for_lang_use, aes(x= caregiver_2_french_use, y = father_french_aoa))+
  geom_jitter()

ggplot(for_lang_use, aes(x= caregiver_1_english_use, y = mother_english_aoa))+
  geom_jitter()

ggplot(for_lang_use, aes(x= caregiver_1_english_use, y = father_english_aoa))+
  geom_jitter()

ggplot(for_lang_use, aes(x= caregiver_2_english_use, y = mother_english_aoa))+
  geom_jitter()

ggplot(for_lang_use, aes(x= caregiver_2_english_use, y = father_english_aoa))+
  geom_jitter()

lm_use_mom_fre <- lm(caregiver_1_french_use ~ mother_french_aoa + father_french_aoa, data= for_lang_use)
lm_use_mom_eng <- lm(caregiver_1_english_use ~ mother_english_aoa + father_english_aoa, data= for_lang_use)

lm_use_dad_fre <- lm(caregiver_2_french_use ~ mother_french_aoa + father_french_aoa, data= for_lang_use)
lm_use_dad_eng <- lm(caregiver_2_english_use ~ mother_english_aoa + father_english_aoa, data= for_lang_use)


summary(lm_use_mom_fre)#The less proficient mother are the less likely they are to use that lang, independent of fathers.
summary(lm_use_mom_eng)#same as avobe but also the less proficient fathers are the more mothers use english.

summary(lm_use_dad_fre) #the less proficient fathers are the less they use fre, but the less proficient mothers are the more dads use french also
summary(lm_use_dad_eng) #same pattern as above.
...
engher_for_sup <- her_for_sup %>%
  filter(bilingual_type == "eng-her")

freher_for_sup <- her_for_sup %>%
  filter(bilingual_type == "fre-her")

##  French aoa
hist(freher_for_sup$mother_french_aoa)
hist(freher_for_sup$father_french_aoa)


## English aoa
hist(engher_for_sup$mother_english_aoa)
hist(engher_for_sup$father_english_aoa)

## heritage aoa
hist(engher_for_sup$mother_heritage_aoa)
hist(engher_for_sup$father_heritage_aoa)
hist(freher_for_sup$mother_heritage_aoa)
hist(freher_for_sup$father_heritage_aoa)


## By strategy accent
##### proficiency (accent) by strategy
   
    her_French_mother_proficiency <- her_for_sup %>%
   filter(!strategy == "single-parent")%>%
   drop_na(strategy)%>%
  group_by(strategy, mother_french_accent) %>%
   drop_na(mother_french_accent)%>%
  ...
  mutate(perc= prop*100)
        ...
        her_French_father_proficiency <- her_for_sup %>%
   filter(!strategy == "single-parent")%>%
   drop_na(strategy)%>%
  group_by(strategy, father_french_accent) %>%
   drop_na(father_french_accent)%>%
  ...
  mutate(perc= prop*100)
    ...
    her_English_mother_proficiency <- her_for_sup %>%
   filter(!strategy == "single-parent")%>%
   drop_na(strategy)%>%
  group_by(strategy, mother_english_accent) %>%
   drop_na(mother_english_accent)%>%
  ...
  mutate(perc= prop*100)
        ...
        her_English_father_proficiency <- her_for_sup %>%
   filter(!strategy == "single-parent")%>%
   drop_na(strategy)%>%
  group_by(strategy, father_english_accent) %>%
   drop_na(father_english_accent)%>%
  ...
  mutate(perc= prop*100)
            ...
            her_heritage_mother_proficiency <- her_for_sup %>%
   filter(!strategy == "single-parent")%>%
   drop_na(strategy)%>%
  group_by(strategy, mother_heritage_accent) %>%
   drop_na(mother_heritage_accent)%>%
  tally()%>%
    mutate(prop = n/sum(n)) %>%
  mutate(perc= prop*100)
    
        her_heritage_father_proficiency <- her_for_sup %>%
   filter(!strategy == "single-parent")%>%
   drop_na(strategy)%>%
  group_by(strategy, father_heritage_accent) %>%
   drop_na(father_heritage_accent)%>%
  tally()%>%
    mutate(prop = n/sum(n)) %>%
  mutate(perc= prop*100)
        
ggplot(her_French_mother_proficiency, aes(y=perc, x=strategy, fill=mother_french_accent)) +
   geom_col(position = "fill") +
   scale_fill_brewer(palette="Dark2")+
   labs(title= "Heriatge sample maternal french proficiency by strategy")+
   theme(axis.text.x = element_text(angle = 25))
 
ggplot(her_French_father_proficiency, aes(y=perc, x=strategy, fill=father_french_accent)) +
   geom_col(position = "fill") +
   scale_fill_brewer(palette="Dark2")+
   labs(title= "Heritage sample paternall french proficiency by strategy")+
   theme(axis.text.x = element_text(angle = 25))

ggplot(her_English_mother_proficiency, aes(y=perc, x=strategy, fill=mother_english_accent)) +
   geom_col(position = "fill") +
   scale_fill_brewer(palette="Dark2")+
   labs(title= "Heritage sample maternal english proficiency by strategy")+
   theme(axis.text.x = element_text(angle = 25))
 
ggplot(her_English_father_proficiency, aes(y=perc, x=strategy, fill=father_english_accent)) +
   geom_col(position = "fill") +
   scale_fill_brewer(palette="Dark2")+
   labs(title= "Heritage sample paternal english proficiency by strategy")+
   theme(axis.text.x = element_text(angle = 25))

ggplot(her_heritage_mother_proficiency, aes(y=perc, x=strategy, fill=mother_heritage_accent)) +
   geom_col(position = "fill") +
   scale_fill_brewer(palette="Dark2")+
   labs(title= "Heritage sample maternal heritage proficiency by strategy")+
   theme(axis.text.x = element_text(angle = 25))
 
ggplot(her_heritage_father_proficiency, aes(y=perc, x=strategy, fill=father_heritage_accent)) +
   geom_col(position = "fill") +
   scale_fill_brewer(palette="Dark2")+
   labs(title= "Heritage sample paternal heritage proficiency by strategy")+
   theme(axis.text.x = element_text(angle = 25))

### proficiency by strategy scatter plots
ggplot(her_for_sup, aes(x= strategy, y = mother_french_aoa))+
  geom_jitter()
ggplot(her_for_sup, aes(x= strategy, y = father_french_aoa))+
  geom_jitter()
ggplot(her_for_sup, aes(x= strategy, y = mother_english_aoa))+
  geom_jitter()
ggplot(her_for_sup, aes(x= strategy, y = father_english_aoa))+
  geom_jitter()
ggplot(her_for_sup, aes(x= strategy, y = mother_heritage_aoa))+
  geom_jitter()
ggplot(her_for_sup, aes(x= strategy, y = father_heritage_aoa))+
  geom_jitter()


######### Proficiency statistical models
#STRATEGY LINEAR MODELS
...
#Summary of STRATEGY LINEAR models
summary(her_lm_stratBYaoa_eng) #no effect of proficiency
summary(her_lm_stratBYaoa_fre) #no effect of proficiency though approaching with mother
summary(her_lm_stratBYaoa_her) #no effect of proficiency 
...
#summary of CAREGIVER Linear models
summary(HERlm_parentsBYaoa_eng) #Father proficiency approaching significance but in the direction of the less proficient the more children hear eng
summary(HERlm_parentsBYaoa_fre) #no proficiency effect
summary(HERlm_parentsBYaoa_her) #no proficiency effect


summary(HERlm_parentsBYacc_eng) #no effect
summary(HERlm_parentsBYacc_fre) #mother accent approaching significant more accented less exposure
summary(HERlm_parentsBYacc_her) #no effect
...
#AOA scatter plots
ggplot(her_for_lang_use, aes(x= caregiver_1_french_use, y = mother_french_aoa))+
  geom_jitter()
ggplot(her_for_lang_use, aes(x= caregiver_1_french_use, y = father_french_aoa))+
  geom_jitter()
ggplot(her_for_lang_use, aes(x= caregiver_2_french_use, y = mother_french_aoa))+
  geom_jitter()
ggplot(her_for_lang_use, aes(x= caregiver_2_french_use, y = father_french_aoa))+
  geom_jitter()
ggplot(her_for_lang_use, aes(x= caregiver_1_english_use, y = mother_english_aoa))+
  geom_jitter()
ggplot(her_for_lang_use, aes(x= caregiver_1_english_use, y = father_english_aoa))+
  geom_jitter()
ggplot(her_for_lang_use, aes(x= caregiver_2_english_use, y = mother_english_aoa))+
  geom_jitter()
ggplot(her_for_lang_use, aes(x= caregiver_2_english_use, y = father_english_aoa))+
  geom_jitter()
ggplot(her_for_lang_use, aes(x= caregiver_1_heritage_use, y = mother_heritage_aoa))+
  geom_jitter()
ggplot(her_for_lang_use, aes(x= caregiver_1_heritage_use, y = father_heritage_aoa))+
  geom_jitter()
ggplot(her_for_lang_use, aes(x= caregiver_2_heritage_use, y = mother_heritage_aoa))+
  geom_jitter()
ggplot(her_for_lang_use, aes(x= caregiver_2_heritage_use, y = father_heritage_aoa))+
  geom_jitter()
...
summary(HERlm_use_mom_fre) #mother proficiency significant the less proficient the less they use
summary(HERlm_use_mom_eng) #father proficiency significant the less proficient fathers are the less mothers use eng but not very significant small effect
summary(HERlm_use_mom_HER) # proficiency not correlated with use. Which does go in line with the idea that mothers take on the role of transmitting heritage language regardless of their proficiency.


summary(HERlm_use_dad_fre) #correlated, the less proficient the less use
summary(HERlm_use_dad_eng) #not correlated
