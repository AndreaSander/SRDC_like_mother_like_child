...
demographic_data <- demographic_data %>%
  #add_count(unique_id) %>%
  #filter(n==1) #HK note: I don't think this line and the one before are doing what you want... this is filtering to IDs that only had an n of 1 in the dataset, but what you want is to remove duplicates, keeping one row for EVERY ID, including the ones that had an n of 2. I have adjusted the code in the next few lines to accomplish this:
  #first, group by ID :
  group_by(unique_id) %>%
...
longitudinal_data<- final_data_strat %>%
  ...
  filter(num_visits>1) #HK note: changed the name of the add_count because earlier in the script the column n was removed, and this way it's more explicit
...
final_data_strat_2opol <- final_data_strat %>% #HK note: the data frame "final_data_language_coded" doesn't exist... changed this to final_data_strat and simplified code below, because it seems like you're doing the same thing to the two datasets except for the strict vs flexible coding.
  mutate(strategy=case_when(
